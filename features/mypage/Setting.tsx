...
export function SettingPage() {
  ...
  return (
    <ScrollView className="flex-1 mt-[80px] bg-white px-5 dark:bg-gray-900 mb-20">
      ...
      {/* 메뉴 - 화살표 있는 항목 */}
      <MenuItem label="프로필 편집" onPress={() => router.push('/mypage/profile-edit')} showArrow />
      <MenuItem label="계정 관리" onPress={() => router.push('/mypage/account')} showArrow />
      <MenuItem label="알림 설정" onPress={() => router.push('/mypage/alert-setting')} showArrow />
      <MenuItem label="제작자 설레게 하기" onPress={() => {}} showArrow />
      <MenuItem label="앱/아이콘 테마" onPress={() => router.push('/mypage/app-theme')} showArrow />
